...
public class MiniTester {
	...
	private String gradeString(int score, int maxScore, String comment) {
		return comment + "Score: " + score * scale + "/" + maxScore * scale;
	}
		...
		try {
			...
			try {
				...
				try {
					...
					res = false;
					comment += "Remove threw exception " + e.toString();
				}
				...
				if (res) {
					try {
						if (myProducts.length == 2 && myProducts[0].equals(r1) && myProducts[1].equals(r3)) {
							grade += 6;
							comment += "Remove worked";
						} else {
							comment += "Remove said it worked but didn't";
						}
					} catch (Exception e) {
						e.printStackTrace();
						comment = comment + e.toString();
					}
				} else {
